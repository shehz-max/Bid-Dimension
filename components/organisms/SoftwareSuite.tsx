        ...
        <div className="flex overflow-hidden">
          ...
          >
            {LOOP_ROW_1.map((tool, idx) => (
              <div
                key={`${tool.name}-${idx}`}
                className="w-[280px] sm:w-[320px] shrink-0 p-4 sm:p-5 bg-white border border-gray-200/90 rounded-2xl shadow-xs hover:border-bd-blue hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex items-center gap-4 group cursor-pointer"
              >
                <div className="w-13 h-13 sm:w-14 sm:h-14 rounded-xl bg-[#F8FAFC] border border-gray-200 p-2 shadow-2xs flex items-center justify-center shrink-0 group-hover:scale-105 transition-transform">
                  <img
                    src={tool.logo}
                    alt={tool.name}
                ...
                <div className="flex flex-col min-w-0">
                  ...
                  <span className="font-body text-xs text-bd-gray truncate">
                    {tool.category}
                  </span>
                </div>
              </div>
